...
The final local dev.7 measurement is 6,707,712 bytes (6.40 MiB) for `Moonmark.exe`, 37,380,674 bytes (35.65 MiB) for the complete portable folder, 16,661,430 bytes (15.89 MiB) for the ZIP, and 13,887,400 bytes (13.24 MiB) for the Inno setup. Exact SHA-256 values are written with each rebuild under `out/release/0.1.0-dev.7/`; do not copy stale hashes into documentation. The remaining folder bytes are Qt, the Windows platform plugin, app-local MSVC CRT, branding, and legal/support files. The packaged smoke passed with only normal Windows system paths visible. This is not yet a clean-VM or minimum-Windows-version certification.
