...
Wolfmark uses one frameless Qt Widgets window with a custom achromatic title bar. The dedicated `WolfTitleBar` component retains Qt's `QWindow::startSystemMove()` path as the portable fallback. On Windows, `windows_window_frame.*` supplies the native non-client contract while Qt continues to own and paint the HWND. Document/core state remains independent.
